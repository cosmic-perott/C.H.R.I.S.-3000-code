...
void shiftClusterCenters() {
  for (int i = 0; i < clusterCenters.length; i++) {
    // Small shift between -2 and +2
    clusterCenters[i][0] += int(random(-2, 3)); // x
    clusterCenters[i][1] += int(random(-2, 3)); // y

    // Clamp to avoid going off the 0–100 logical grid
    clusterCenters[i][0] = constrain(clusterCenters[i][0], 10, 90);
    clusterCenters[i][1] = constrain(clusterCenters[i][1], 10, 90);
  }
}

void generateNewPoints() {shiftClusterCenters();    // <-- Shift centers before generating new data
   circles.clear();  // <-- remove all previously stored circles
  ...
  for (int i = 0; i < numPoints; i++) {
    ...
    float sy = map(y, 0, 100, height, 0); // Flip Y
