...
void setup() {
  ...
  for (int i = 0; i < numPoints; i++) {
    ...
    float x = constrain((float) (randomGaussian() * 5 + center[0]), 0, 100);
    float y = constrain((float) (randomGaussian() * 5 + center[1]), 0, 100);
    
    // Map 0-100 to screen size
    float sx = map(x, 0, 100, 0, width);
    float sy = map(y, 0, 100, height, 0);  // Flip Y-axis like matplotlib
...
class Circle {
  ...
  void updateColor() {
    if (radius <= 2) {
      c = color(0, 0, 255, 90); // Blue - more transparent
    } else if (radius <= 5) {
